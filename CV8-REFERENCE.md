...
With 35 direct and 37 escaped primitives (Iteration 260):
...
| `0x7E` | `ESC` + a selector byte: 37 OS/libc primitives of 256 selectors |
| `0x7F` | free |
| `0x80`–`0xFF` | first byte of a two- or three-byte call |

**Escaped primitives cost no opcode.** The 37 OS/libc primitives —
`BYE`, the file and process words, `READ`, `WRITE`, `POLL`,
`RAW-MODE`, and since Iteration 260 `MOVE`, `FILL`, `COMPARE`, `SCAN`
and `CSTRLEN` (memmove, memset, memcmp, memchr, strlen) — are

...
static void gen(void) {
    ...
    for (unsigned i = 0; i < NOPS; i++) {
        ...
        if (r < 25) {
            ...
            arg[i] = (t < 450) ? (t % 32) : 32 + (t * 37 + i) % NWORDS;
...
static unsigned char fx[NOPS * 4], vi[NOPS * 4], vi2[NOPS * 4];
static unsigned n_cell, n_fx, n_vi;
...
static uintptr_t wordtab[NWORDS + 64];
...
static double run_fixed(void) {
    ...
    for (int r = 0; r < REPS; r++)
        for (unsigned i = 0; i < n_fx; i++) {
            ...
            else { unsigned v = ((b & 0x7F) << 8) | fx[++i]; WORK(wordtab[v % NWORDS] + v); }
        }
    return now() - t0;
}
...
int main(void) {
    gen(); encode();
    for (unsigned i = 0; i < NWORDS + 64; i++) wordtab[i] = i * 3;
    double bc = 1e30, bf = 1e30, bv = 1e30, bv2 = 1e30, t;
    ...
    printf("  vs fixed: varint %.3f  peeled %.3f     (sink %llu)\n",
           bv / bf, bv2 / bf, (unsigned long long)sink);
    return 0;
}
